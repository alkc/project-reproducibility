...
bikeshare %>% 
  ...
  geom_violin() 
```
The violin plot suggests that total number of rides for class 3 bad weathers lean more towards lower counts.
...
No idea about that spring effect. Would expect winter to look like spring does.

```{r}
bikeshare %>% 
  ggplot(aes(x = working_day, y = total_nbr_rentals)) +
  geom_violin()
```
```{r}
bikeshare %>% 
  ggplot(aes(x = holiday, y = total_nbr_rentals)) +
  geom_violin()
```
Next, visualize total number of rentals by daily hour -- the new variable introduced earlier in code. 
...
```
Create new boolean variable `rush_hour`:
...
#### Exploration of quantitative predictors
...
```{r}
bikeshare %>% 
  ggplot(aes(x = feels_like_temp_celcius, y = total_nbr_rentals)) + 
  geom_point(alpha = 0.1) +
  geom_smooth() +
  facet_wrap(~hour)
```
Next up, wind speed. Personal experience of biking to and from Malmö leaves me strongly prejudiced for this variable 
having some sort of effect in data:
...
bikeshare %>% 
  ...
  geom_smooth() 
```
Any difference for high wind speeds in working/non working days?

```{r}
bikeshare %>% 
  ggplot(aes(x = wind_speed, y = total_nbr_rentals, color = rush_hour)) + 
  geom_jitter(alpha = 0.1) +
  facet_wrap(~working_day)
```
...
We will do a 5-fold CV to figure out the best configuration of random forest for our data. Probably should use a higher
k-fold with repeats. This takes too much time though.
...
```
As seen in exploratory plots: hour of day and whether or not it is rush hour are important factors when predicting 
